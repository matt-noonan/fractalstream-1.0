...
- (void*) emit: (int) node {
	...
	/*** active modes ***/
		...
		if(tree[node].cachedAt && (tree[node].cachePass == mode)) {  // don't reprocess if we've already done it
					...
					case FSE_If:
						{
							...
							builder -> SetInsertPoint(falseBlock);
								if(tree[node].auxi[0] != -1) [self emit: tree[node].auxi[0]];
								builder -> CreateBr(mergeBlock);
...
*/
			...
			case FSE_Func:
				switch(tree[node].type & (-1 ^ FSE_Type_Mask)) {
					...
					case FSE_Sin:
						{
							NSLog(@"about to make an sin node\n");
							lhs = (Value*) [self emit: tree[node].firstChild];
							std::vector<Value*> args;
							args.push_back(lhs);
